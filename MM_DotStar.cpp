 ...
 * @file MM_DotStar.cpp
 *
 * Forked from the Adafruit library, to add choosable SPI port support
 *
 ...
 *
 * This file is part of the MM_DotStar library, forked from the Adafruit_DotStar library.
 *
...
*/
MM_DotStar::MM_DotStar(uint16_t n, uint8_t o)
...
MM_DotStar::MM_DotStar(uint16_t n, SPIClass* spi, uint8_t o) :
 numLEDs(n), dataPin(USE_HW_SPI), brightness(0), pixels(NULL),
 rOffset(o & 3), gOffset((o >> 2) & 3), bOffset((o >> 4) & 3), hwSPI(spi)
{
  updateLength(n);
...
/*!
  ...
  @return  Adafruit_DotStar object. Call the begin() function before use.
*/
MM_DotStar::MM_DotStar(uint16_t n, uint8_t data, uint8_t clock,
                                   uint8_t o)
...
/*!
  @brief   Switch over to hardware SPI. DotStars must be connected to
           ...
           continue to be used.
*/
void MM_DotStar::updatePins(void) {
  sw_spi_end();
  dataPin = USE_HW_SPI;
  hw_spi_init();
}
...
/*!
  ...
  @param   clock  Arduino pin number for clock out.
*/
void MM_DotStar::updatePins(uint8_t data, uint8_t clock) {
  hw_spi_end();
  dataPin = data;
  clockPin = clock;
  sw_spi_init();
...
/*!
  ...
  @note    This function is deprecated, here only for old projects that
           ...
           'new' keyword.
*/
void MM_DotStar::updateLength(uint16_t n) {
  free(pixels);
...
*/
void MM_DotStar::hw_spi_init(void) { // Initialize hardware SPI
...
*/
void MM_DotStar::hw_spi_end(void) {
#ifdef __AVR_ATtiny85__
...
/*!
  @brief   Initialize 'soft' (bitbang) SPI. Data and clock pins are set
           to outputs.
*/
void MM_DotStar::sw_spi_init(void) {
...
#define spi_out(n) (void)hwSPI->transfer(n) ///< Call hardware SPI function
...
*/
void MM_DotStar::sw_spi_out(uint8_t n) {
...
*/
void MM_DotStar::setPixelColor(uint16_t n, uint8_t r, uint8_t g,
                                     uint8_t b) {
  if (n < numLEDs) {
...
/*!
  ...
  @param   c  32-bit color value. Most significant byte is 0, second is
              ...
              e.g. 0x00RRGGBB
*/
void MM_DotStar::setPixelColor(uint16_t n, uint32_t c) {
  if (n < numLEDs) {
    uint8_t *p = &pixels[n * 3];
...
*/
void MM_DotStar::fill(uint32_t c, uint16_t first, uint16_t count) {
  uint16_t i, end;
...
*/
uint32_t MM_DotStar::ColorHSV(uint16_t hue, uint8_t sat, uint8_t val) {
...
/*!
  ...
  @return  'Packed' 32-bit RGB value. Most significant byte is 0, second is
           is red, then green, and least significant byte is blue.
*/
uint32_t MM_DotStar::getPixelColor(uint16_t n) const {
  if (n >= numLEDs)
    return 0;
  uint8_t *p = &pixels[n * 3];
...
/*!
  ...
  @note    For various reasons I think brightness is better handled in
           ...
           stored.
*/
void MM_DotStar::setBrightness(uint8_t b) {
...
*/
uint8_t MM_DotStar::getBrightness(void) const {
...
/*!
  ...
  @return  Gamma-adjusted packed color, can then be passed in one of the
           ...
           function instead.
*/
uint32_t MM_DotStar::gamma32(uint32_t x) {
  uint8_t *y = (uint8_t *)&x;
